...
describe('Desafios iniciais', () => {
  ...
  describe.only('Escreva uma query que exiba os valores da coluna que representa a primary key da tabela `products`', () => {
  ...
  describe.only('Conte quantos registros existem em `product_name` de `products`', () => {
  ...
  describe.only('Monte uma query que exiba os dados da tabela `products` a partir do quarto registro até o décimo terceiro, incluindo tanto um quanto o outro', () => {
  ...
  describe.only('Exiba os dados das colunas `product_name` e `id` da tabela `products` de maneira que os resultados estejam em ordem alfabética dos nomes', () => {
  ...
  describe.only('Mostre apenas os ids dos 5 últimos registros da tabela `products` (a ordernação deve ser baseada na coluna `id`)', () => {
  ...
  describe.only('Faça uma consulta que retorne três colunas contendo os nomes `A`, `Trybe` e `eh` com os valores `5 + 6`, `"de"` e `2 + 8`, respectivamente', () => {
